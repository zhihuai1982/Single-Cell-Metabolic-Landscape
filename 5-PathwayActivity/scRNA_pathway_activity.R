...
mean_expression_shuffle <- matrix(NA,
...
mean_expression_noshuffle <- matrix(NA,
    nrow = length(pathway_names),
    ncol = length(cell_types),
    dimnames = list(
        pathway_names,
...
for (p in pathway_names) {
    ...
    mean_exp_eachCellType <- apply(
        pathway_metabolic_tpm, 1,
        function(x) {
            by(x, all_cell_types, mean)
        }
    )
    ...
    ratio_exp_eachCellType <- t(mean_exp_eachCellType) / colMeans(mean_exp_eachCellType)
    mean_exp_pathway <- apply(
        ratio_exp_eachCellType,
        2,
        function(x) {
            weighted.mean(
                x,
                pathway_number_weight / sum(pathway_number_weight)
            )
        }
    )
    ...
    group_mean <- function(x) {
        sapply(
            cell_types,
            function(y) {
                rowMeans(pathway_metabolic_tpm[, shuffle_cell_types_list[[x]] == y, drop = F ])
            }
        )
    }
    ...
    ratio_exp_eachCellType_list <- lapply(
        times,
        function(x) mean_exp_eachCellType_list[[x]] / rowMeans(mean_exp_eachCellType_list[[x]])
    )
...
g <- ggplot(scRNA_df, aes(x = variable, y = value, fill = variable)) +
    ...
    geom_violin(trim = F, size = 0.2, show.legend = F, width = 1.0) +
    labs(y = NULL, x = NULL) +
