...
for (selected_t in tumors) {
    ...
    pvalues <- sapply(
        X = seq_len(nrow(each_tumor_tpm_selected)),
        FUN = function(x) {
            return(wilcox.test(as.numeric(unlist(each_tumor_tpm_selected[x, ])) ~ condition, alternative = "greater")$p.value)
        }
    )
...
pvalues <- sapply(
    X = 1:nrow(selected_tumor_tpm_selected),
    FUN = function(x) {
        return(wilcox.test(as.numeric(unlist(selected_tumor_tpm_selected[x, ])) ~ condition, alternative = "greater")$p.value)
    }
)
